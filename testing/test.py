...
sys.path.insert(0, str(ROOT))  # Ensure package is importable
import ramantools as rt  # Direct import; indentation harmonized in source
...
# ---------------------------------------------------------------------------
try:
    m = rt.ramanmap(MAP_PATH, MAP_INFO)  # Load map for reuse
    logger.info('load ramanmap passed')
except Exception as exc:
    failures.append('load ramanmap')
    logger.error('load ramanmap failed: %s', exc)
    m = None
try:
    s = rt.singlespec(SPEC_PATH, SPEC_INFO)  # Load spectrum for reuse
    logger.info('load singlespec passed')
except Exception as exc:
    failures.append('load singlespec')
    logger.error('load singlespec failed: %s', exc)
    s = None
if (m is None) or (s is None):
    logger.error('Aborting subsequent tests due to earlier load failure')
else:
